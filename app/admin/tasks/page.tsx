  ...
  return (
    <div className="flex min-h-screen bg-slate-50">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-500">
              Curate 90-day task schedules, reading resources, timestamps, and task templates
            </p>
          </div>
                        ...
                        <div key={week.id} className="p-4 bg-slate-50 rounded-xl border border-slate-200/80 space-y-2">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5 pt-2">
                            {week.days?.map((day: any) => (
                              <div key={day.id} className="p-3 bg-white rounded-lg border border-slate-200 text-xs space-y-1">
                                <div className="flex items-center justify-between font-bold text-slate-900">
                                  <span>Day {day.dayNumber}: {day.title || `Day ${day.dayNumber}`}</span>
                                  <span className="text-[10px] font-semibold text-slate-500">{day.tasks?.length || 0} Tasks</span>
                                </div>
                                {day.tasks?.map((t: any) => (
                                  <div key={t.id} className="text-[11px] text-slate-600 flex items-center justify-between bg-slate-50 px-2 py-1 rounded border border-slate-100">
                                    <span className="truncate max-w-[240px] font-medium">{t.title}</span>
            ...
            {activeTab === 'TEMPLATES' && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {treeData?.templates?.map((tmpl: any) => (
                  <Card key={tmpl.id} variant="glass" className="space-y-2 p-4">
                    <div className="flex items-center justify-between">
            ...
            {activeTab === 'RESOURCES' && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {treeData?.resources?.map((res: any) => (
                  <Card key={res.id} variant="glass" className="space-y-2 p-4">
                    <div className="flex items-center justify-between">
                    ...
                    {res.url && (
                      <a href={res.url} target="_blank" rel="noopener noreferrer" className="text-xs font-bold text-brand-600 hover:underline block truncate">
                        {res.url}
                      </a>
                    )}
                  </Card>
      ...
      {showTaskModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <Card variant="glass" className="max-w-xl w-full p-6 space-y-4 bg-white my-8">
            <h3 className="text-lg font-bold text-slate-900">Assign / Create Task for Programme Day</h3>
      ...
      {showTemplateModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <Card variant="glass" className="max-w-md w-full p-6 space-y-4 bg-white">
            <h3 className="text-lg font-bold text-slate-900">Create Reusable Task Template</h3>
      ...
      {showResourceModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <Card variant="glass" className="max-w-md w-full p-6 space-y-4 bg-white">
            <h3 className="text-lg font-bold text-slate-900">Add Curated Resource</h3>
