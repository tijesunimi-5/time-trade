...
import { Users, AlertTriangle, CheckCircle, Clock, MessageSquare, Phone, Plus, X } from 'lucide-react';
    ...
    <div className="flex min-h-[calc(100vh-65px)]">
      ...
      <div className="flex-1 p-4 lg:p-8 space-y-8 max-w-6xl mx-auto">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl font-black text-white">Assigned Cohort Monitoring</h1>
          <p className="text-xs text-slate-400">Surface factual activity trends to reach out to participants effectively</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <Card variant="glass" className="space-y-1">
            <span className="text-[10px] font-bold text-amber-400 uppercase tracking-widest">Needs Attention</span>
          ...
          <Card variant="glass" className="space-y-1">
            <span className="text-[10px] font-bold text-rose-400 uppercase tracking-widest">Inactive (&gt;48h)</span>
        ...
        <Card variant="glass" className="p-0 overflow-hidden">
          <div className="p-4 border-b border-white/10 flex items-center justify-between">
            <h3 className="text-base font-bold text-white flex items-center gap-2">
              <Users className="w-4 h-4 text-emerald-400" /> Assigned Sub-Group Roster
            </h3>
            <span className="text-xs text-slate-400">Showing {participants.length} assigned participants</span>
          </div>

          {isLoading ? (
            <div className="p-12 text-center text-slate-400">Loading assigned participants...</div>
          ) : (
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-white/10 text-[10px] font-bold text-slate-400 uppercase tracking-widest bg-navy-900/60">
                    ...
                    <th className="p-4">Factual Status</th>
                  ...
                  {participants.map((p) => (
                    <tr key={p.id} className="hover:bg-slate-800/40 transition-colors">
                      <td className="p-4 pl-6 font-bold text-white flex items-center gap-3">
                        ...
                        />
                        <div>
                          <span className="block font-bold">{p.fullName}</span>
                          <span className="text-xs text-slate-400 font-normal">{p.email}</span>
                        </div>
                      </td>
                      <td className="p-4 font-bold text-brand-cyan">{p.todayProgress}</td>
                      <td className="p-4 font-semibold text-slate-300">{p.weeklyProgress}</td>
                      <td className="p-4 text-xs font-mono text-slate-400">{p.lastActive}</td>
                      <td className="p-4">
                        <Badge
                          variant={
                            p.status === 'ACTIVE'
        ...
        {selectedParticipant && (
          <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
            <Card variant="glass" className="max-w-2xl w-full p-6 space-y-6 relative max-h-[90vh] overflow-y-auto">
              <button
              ...
              <div className="flex items-center gap-4 border-b border-white/10 pb-4">
                ...
                />
                <div>
                  <h3 className="text-xl font-bold text-white">{selectedParticipant.fullName}</h3>
                  <p className="text-xs text-slate-400">{selectedParticipant.email} • {selectedParticipant.phone}</p>
              ...
              <div className="flex items-center justify-between p-4 rounded-xl bg-emerald-950/40 border border-emerald-500/30">
                <span className="text-xs text-emerald-300 font-medium">Reach out via WhatsApp or Phone call:</span>
                <a
              ...
              <div className="space-y-3">
                <h4 className="text-sm font-bold text-white uppercase tracking-wider">Follow-up Activity Ledger</h4>

                <form onSubmit={handleAddNote} className="space-y-2">
                  <textarea
                    placeholder="Add factual follow-up note (e.g., Contacted via WhatsApp on 10 AM, confirmed sickness)..."
                    rows={2}
                    value={noteContent}
                    onChange={(e) => setNoteContent(e.target.value)}
                    className="w-full px-3 py-2 rounded-xl glass-input text-xs font-medium placeholder:text-slate-500"
                  />
                ...
                <div className="space-y-2 pt-2">
                  {selectedParticipant.notes && selectedParticipant.notes.length > 0 ? (
                    selectedParticipant.notes.map((note: any) => (
                      <div key={note.id} className="p-3 rounded-xl bg-navy-900/60 border border-white/5 space-y-1">
